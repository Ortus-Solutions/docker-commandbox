...
Quick and Dirty Feed Dump:
	...
	<cfset rc.webfeed = getModel("FeedReader@cbfeeds").retrieveFeed("http://www.example.com/feeds/rss")>
	<cfdump var="#rc.webfeed#">
	<cfabort>

----------------------------------------------------------------------->
<cfcomponent hint="A feed reader plug-in that processes Atom, RDF and RSS formats. The recommended method for general usage is readFeed()."
			 output="false"
			 singleton>
...
<!---------------------------------------- CONSTRUCTOR --------------------------------------------------->
	...
	<cffunction name="init" access="public" returntype="FeedReader" output="false" hint="Constructor">
		...
		<cfscript>
			...
			setCacheTimeout( arguments.settings.cacheTimeout );
			sethttpTimeout( arguments.settings.httpTimeout);
			...
			if( getUseCache() ){
				...
				if( getCacheType() eq "file" ){
					...
					setCachePrefix( '' );
					/* Validate it */
					if( !directoryExists( arguments.settings.cacheLocation ) ){
						throw( message="The cache location directory could not be found, please check again: '#arguments.settings.cacheLocation#' ",
							   type='FeedReader.InvalidCacheLocationException');
					}
					/* Set the location */
...
<!---------------------------------------- INTERNAL CACHE OPERATIONS ------------------------------------->
	...
	<cffunction name="flushCache" output="false" access="public" returntype="void" hint="Flushes the entire file cache by removing all the entries">
		...
		<cfif getCacheType() eq "ram">
			<cfset variables.cache.clearByKeySnippet( getCachePrefix() )>
		<cfelse>
	...
	<cffunction name="isFeedCached" output="false" access="public" returntype="boolean" hint="Checks if a feed is cached or not">
		...
		<cfif getCacheType() eq "ram">
			<cfreturn variables.cache.lookup(URLToCacheKey(arguments.feedURL))>
		<cfelse>
			<!--- Secure cache read --->
			<cflock name="#getLockName()#" type="readonly" timeout="30" throwontimeout="true">
	...
	<cffunction name="removeCachedFeed" output="false" access="public" returntype="boolean" hint="Purges a feed from the cache, returns false if feed is not found">
		...
		<cfif getCacheType() eq "ram">
			<cfreturn variables.cache.clear(URLToCacheKey(arguments.feedURL))>
		<cfelse>
			<!--- Cache file --->
	...
	<cffunction name="getCachedFeed" output="false" access="public" returntype="any" hint="Get the feed content from the cache, if missing a blank structure is returned. This method does NOT timeout or expire the feeds that is only done by the readFeed method.">
		...
		<cfset var objectIn = "">
		<cfif getCacheType() eq "ram">
			<cfset results = variables.cache.get(URLToCacheKey(arguments.feedURL))>
		<cfelse>
			<cfset cacheFile = getCacheLocation() & "/" & URLToCacheKey(arguments.feedURL)>
			<!--- Secure cache read --->
			<cflock name="#getLockName()#" type="exclusive" timeout="30" throwontimeout="true">
	...
	<cffunction name="setCachedFeed" output="false" access="public" returntype="void" hint="Copy feed content into the cache">
		...
		<cfif getCacheType() eq "ram">
			<cfset variables.cache.set(cacheKey, feedStruct, getCacheTimeout())>
		<cfelse>
...
<!---------------------------------------- PUBLIC FEED METHODS ------------------------------------------->
				...
				if(ListLen(arguments.feedURL) gt 1) {
					...
					if(i lte 1) {
						...
						for (j=1;j lte ArrayLen(feedCombArr[i].category);j=j+1) {
							...
							feed.category = createObject('component','util.ReaderUtil').arrayOfStructsSort(feed.category,'tag');
						...
						if( arguments.itemsType eq "array" ) {
							...
							feed.items = createObject('component','util.ReaderUtil').arrayOfStructsSort(feed.items,'datepublished');
						...
						// when arguments itemType is 'query', ...
							...
							// append new feed items to master items
							feed.items = doQueryAppend( feed.items, feedCombArr[i].items );
							// reorder and trim the master items
							feed.items = createObject('component','util.ReaderUtil').querySortandTrim(feed.items,arguments.maxItems,'datepublished','desc');
						}
					}
	...
	<cffunction name="parseFeed" access="public" returntype="struct" hint="This parses a feed as a XML document and returns the results as a structure of elements" output="false">
		...
		<cfset var oUtilities = createObject( "component", "util.DateUtils" )>
		<cfset var extras = createObject('component','util.ReaderUtil').init()>
		<cfscript>
...
<!---------------------------------------- PRIVATE METHODS ----------------------------------------------->
	...
	<cffunction name="createUserAgent" access="package" output="false" returntype="string" hint="Creates a ColdBox user agent used in HTTP requests">
		<cfscript>
			var ua = "CBOX-FEEDS/";
			ua = ua & ' (#server.coldfusion.productname# #server.coldfusion.productversion#)'; // CFML engine and operating system
			return ua;
		</cfscript>
	</cffunction>
